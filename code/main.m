...
for i=1:size(Files_cell,1) % for each point cloud representing a building
    ...
    if size(Files,1)>2
        ...
        if size(xyz,1)>50           
            ...
            for s=1:size(segment,1)
                ...
                % in case the angle phi is > of 70 degrees, the plane is
                % not a roof pitch
                if angles(s,2)>70 
                    lab_vert=[lab_vert; s];
                    mfeFINALE(end)=[];
            ...
            % in case the angle phi is > of 70 degrees, the plane is
            % not a roof pitch, so it should not be considered
            segmentation={};
            ...
            if size(segment,1)>1
                ...
                % maximal aggregation of segments
                segm=cell(max(T),1);
            ...
            % for each plane, compute the external boundary polygon 
            polygons={};
            lab_pol=[];
            panel_tresh=1.7;
            for s=1:size(segment,1)
                ...
                if size(aus,1)>20
                    ...
                    if  dist1> panel_tresh && dist2>panel_tresh 
                        z=mean(aus(:,3));
                        % compute the boundary
                        k=boundary(aus(:,1),aus(:,2),0.8);
                        % refine the boundary through the DP algorithm
            ...
            % create .txt files cotaining the angles associated to each
            % roof pitch
            angle_aus=angle_fin;
